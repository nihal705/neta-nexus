...
import { 
  ...
  FiUserCheck, FiFileText, FiClock, FiStar, FiBarChart2,
  FiCheckCircle, FiXCircle, FiUser, FiMail, FiPhone,
  FiGlobe, FiPhoneCall, FiTwitter, FiFacebook, FiLinkedin
} from 'react-icons/fi'
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="space-y-4">
          ...
          <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4">
            ...
            <div className="text-xl font-bold">₹{movable.toLocaleString()}</div>
          </div>
          <div className="bg-purple-50 dark:bg-purple-900/20 rounded-xl p-4">
            <div className="text-sm text-gray-500">Immovable Assets</div>
            <div className="text-xl font-bold">₹{immovable.toLocaleString()}</div>
          </div>
        </div>
        <div className="space-y-4">
          <div className="bg-red-50 dark:bg-red-900/20 rounded-xl p-4">
            <div className="text-sm text-gray-500">Total Liabilities</div>
            <div className="text-xl font-bold text-red-600">{affidavit.total_liabilities || 'None'}</div>
          </div>
          <div className="bg-yellow-50 dark:bg-yellow-900/20 rounded-xl p-4">
            <div className="text-sm text-gray-500">Annual Income</div>
            <div className="text-xl font-bold">{affidavit.annual_income || 'N/A'}</div>
          </div>
        </div>
      </div>
    </div>
...
// Criminal Record Component
const CriminalRecord = ({ cases }) => {
...
// Parliamentary Score Component
const ParliamentaryScore = ({ score }) => {
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        <div className="text-sm text-gray-500">Composite Performance Score</div>
        <div className={`text-5xl font-bold ${getScoreColor(score.composite_score)}`}>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 text-center">
          <div className="text-2xl font-bold text-primary-teal">{score.attendance_percentage}%</div>
          <div className="text-xs text-gray-500">Attendance</div>
        </div>
        <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 text-center">
          <div className="text-2xl font-bold text-primary-teal">{score.questions_asked}</div>
          <div className="text-xs text-gray-500">Questions Asked</div>
        </div>
        <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 text-center">
          <div className="text-2xl font-bold text-primary-teal">{score.debates_participated}</div>
          <div className="text-xs text-gray-500">Debates</div>
        </div>
        <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 text-center">
          <div className="text-2xl font-bold text-primary-teal">{score.private_member_bills}</div>
...
export default function CandidateProfilePage() {
  ...
  if (!candidate) {
    return (
      <div className="text-center py-12">
        ...
        <Link to="/" className="text-primary-teal hover:underline mt-4 inline-block">
          ← Back to Home
        </Link>
      </div>
    )
  ...
  const tabs = [
    { id: 'overview', label: 'Overview', icon: <FiUserCheck size={18} /> },
    { id: 'affidavit', label: 'Assets & Affidavit', icon: <FiFileText size={18} /> },
    { id: 'criminal', label: 'Criminal Records', icon: <FiAlertCircle size={18} /> },
    { id: 'elections', label: 'Election History', icon: <FiTrendingUp size={18} /> },
    { id: 'performance', label: 'Parliamentary Score', icon: <FiStar size={18} /> },
  ]
  ...
  const getAvatarColor = () => {
    ...
    return colors[(candidate.id || 0) % colors.length]
  }
  ...
  return (
    ...
    >
      ...
      <Link to="/search" className="inline-flex items-center gap-2 text-gray-500 hover:text-primary-teal mb-4 transition">
        <FiChevronLeft size={18} /> Back to Search
      </Link>
        ...
        <div className="bg-gradient-to-r from-primary-navy via-primary-blue to-primary-teal px-6 py-8">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div className="flex items-center gap-4">
              {/* Avatar */}
              <div className={`w-20 h-20 bg-gradient-to-br ${getAvatarColor()} rounded-full flex items-center justify-center text-white text-3xl font-bold shadow-lg`}>
                {getInitials()}
              </div>
              <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-primary-teal/10 rounded-full flex items-center justify-center">
                <FiCalendar className="text-primary-teal" size={18} />
              </div>
              <div>
            ...
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-primary-teal/10 rounded-full flex items-center justify-center">
                <FiMapPin className="text-primary-teal" size={18} />
              </div>
              <div>
            ...
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-primary-teal/10 rounded-full flex items-center justify-center">
                <FiBriefcase className="text-primary-teal" size={18} />
              </div>
              <div>
            ...
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-primary-teal/10 rounded-full flex items-center justify-center">
                <FiBookOpen className="text-primary-teal" size={18} />
              </div>
              <div>
      ...
      <div className="space-y-6">
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6">
              <h3 className="text-lg font-semibold mb-4 flex items-center gap-2">
                <FiUserCheck className="text-primary-teal" /> Profile Summary
              </h3>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6">
              <h3 className="text-lg font-semibold mb-4 flex items-center gap-2">
                <FiBarChart2 className="text-primary-teal" /> Quick Statistics
              </h3>
              <div className="grid grid-cols-2 gap-4">
        ...
        {activeTab === 'affidavit' && candidate.affidavit && (
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6">
            <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
              <FiAward className="text-primary-teal" /> Assets Declaration
            </h2>
            <AssetChart affidavit={candidate.affidavit} />
        ...
        {activeTab === 'criminal' && (
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6">
            ...
            <CriminalRecord cases={candidate.criminal_cases || []} />
          </div>
        )}
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                  {candidate.election_results.map((result, idx) => (
                    <tr key={idx} className="hover:bg-gray-50 dark:hover:bg-gray-800 transition">
                      ...
                      <td className="px-4 py-3">{result.election_type || 'General'}</td>
                      ...
                      <td className="px-4 py-3">{result.margin?.toLocaleString()}</td>
                      <td className="px-4 py-3">
                        {result.winner ? (
        ...
        {activeTab === 'performance' && (
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6">
            ...
            {candidate.parliamentary_score ? (
              <ParliamentaryScore score={candidate.parliamentary_score} />
            ) : (
              <div className="text-center py-8 text-gray-500">
                <FiClock className="text-4xl mx-auto mb-2" />
                <p>Parliamentary performance data not available for this candidate</p>
                <p className="text-sm mt-1">This candidate may be from State Assembly or data not yet updated</p>
              </div>
            )}
          </div>
        )}
